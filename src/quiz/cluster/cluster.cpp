...
void proximity(int index, std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
	processed[index] = true;
  	...
  	std::vector<int> nearby_point = tree->search(points[index], distanceTol);
  	
  	for(int nearby_id: nearby_point)
    {
    	if(!processed[nearby_id])
          	proximity(nearby_id, points, cluster, processed, tree, distanceTol);
    }
}
